...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_AUTH_CMPL_EVT:
        ...
        badge_ble_bonded(param->ble_security.auth_cmpl.bd_addr);
    ...
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT: // ESP_IO_CAP_OUT
        ...
        badge_ble_passkey(param->ble_security.key_notif.passkey);
        break;
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    if ((ret = esp_ble_gap_config_adv_data(&ble_scan_rsp)) != ESP_OK) {
        ESP_LOGE(TAG, "GAP config scan_rsp failed: %d", ret);
        return ret;
    }
